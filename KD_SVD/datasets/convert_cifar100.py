...
dataset_dir = './cifar-100-python-32-4'
# SIZE = 32
SIZE = 16
...
def resize(image):
    ...
    if SIZE != 32:
        image = Image.fromarray(image).resize((SIZE, SIZE), Image.ANTIALIAS)
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.python_io.TFRecordWriter(training_filename) as tfrecord_writer:
            ...
            with tf.Session(
                ...
                dataset_len = 0
                with open(os.path.join(dataset_dir, 'train'), 'rb') as fo:
        ...
        with tf.python_io.TFRecordWriter(test_filename) as tfrecord_writer:
            ...
            with tf.Session(
                    ...
                    config=tf.ConfigProto(log_device_placement=True)) as sess:
                dataset_len = 0
                with open(os.path.join(dataset_dir, 'test'), 'rb') as fo:
                    img_queue = pickle.load(fo, encoding='bytes')
