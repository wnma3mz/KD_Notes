...
def _get_output_filename(dataset_dir, split_name):
    return '%s/imgnet32%s.tfrecord' % (dataset_dir, split_name)
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.python_io.TFRecordWriter(test_filename) as tfrecord_writer:
            ...
            with tf.Session(
                    ...
                    config=tf.ConfigProto(log_device_placement=True)) as sess:
                dataset_len = 0
                with open('/home/dmsl/Documents/data/Imagenet32/val_data',
                          'rb') as fo:
                    img_queue = pickle.load(fo, encoding='bytes')
