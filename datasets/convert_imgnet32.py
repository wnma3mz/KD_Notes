    ...
    with tf.device('/cpu:0'):
        with tf.python_io.TFRecordWriter(training_filename) as tfrecord_writer:
            ...
            with tf.Session('',config=tf.ConfigProto(log_device_placement=True)) as sess:
                dataset_len = 0
                for i in range(1,11):
                    with open('/home/dmsl/Documents/data/Imagenet32/train_data_batch_%d'%i, 'rb') as fo:
                        img_queue = pickle.load(fo, encoding='bytes')
                    
                    for n in xrange(img_queue['data'].shape[0]):
                        image = np.transpose(img_queue['data'][n].reshape(3,32,32),[1,2,0])[:,:,(2,1,0)]
        ...
        with tf.python_io.TFRecordWriter(test_filename) as tfrecord_writer:
            ...
            with tf.Session('',config=tf.ConfigProto(log_device_placement=True)) as sess:
                dataset_len = 0
                with open('/home/dmsl/Documents/data/Imagenet32/val_data', 'rb') as fo:
                    img_queue = pickle.load(fo, encoding='bytes')
